...
> **Superseded (2026-07-08):** the board↔issue sync sections below were re-architected under `docs/active-spikes/board-sync-redesign.md` (see `docs/decisions/0005`). Net changes: `kanban-status-reopen`/`open-roles-reopen` merged into one `board-reopen-reconcile.yaml` (5-min, both boards); `close-to-done`/`closed-to-filled` kept as the code path for issue→Done/Filled (native "Item closed" is now OFF, so they're no longer redundant); `open-role-add` now sets Open status in code. Entries below are historical; the redesign spike is authoritative for current behavior.
...
- [x] **Bug fix (2026-07-08):** `stale-role-warning.yaml` had never run once — confirmed via run history (every invocation a `push` validation failure, never a successful `schedule` run). The `gh issue comment --body "..."` multi-line string had continuation lines at column 1, which de-dents out of the `run: |` YAML block scalar and makes `**` parse as a broken YAML alias — the whole workflow failed to load. Fixed by building the body with `printf` (indented, reusing `$WARNING_MARKER`). This is the same class of "looks fine, silently never runs" failure as the `projects_v2_item` bug; worth a `ruby -ryaml` (or equivalent) parse check on any workflow with an inline multi-line comment body.
- [x] **Security fix (2026-07-08):** `label-open-role-from-form.yaml` inlined `${{ github.event.issue.body }}` directly into a shell heredoc — a script-injection vector (a crafted issue body could run arbitrary commands as the `ACTIONS_TOKEN` bot). Fixed by passing the body through an `env:` var (`ISSUE_BODY`) instead. Standard GitHub Actions hardening.

### Design observations (not yet actioned — for user decision)

- **`close-to-done.yaml` is redundant** with the Org Kanban's native "Item closed" built-in workflow (both set Status→Done when an issue closes; the native one is enabled and confirmed working). Not harmful (idempotent), but it's duplicate maintenance. Candidate for deletion, same as `filled-to-close`/`done-to-close` — left in place pending a decision because, unlike those two, it *does* actually run.
- **Scheduled reopen jobs enforce a standing invariant, not a transition.** `kanban-status-reopen`/`open-roles-reopen` reopen *any* closed issue whose card sits in an active column, re-checked every 15 min. If someone deliberately closes an issue but leaves its card in To Do/Open, the job will fight them and reopen it repeatedly. In normal flow this can't happen (closing routes the card to Done/Filled via native/`closed-to-filled`), but it's a sharper edge than the old event-driven model. Documented in `docs/decisions/0004`.
- **Native "Auto-add to project" is enabled on Org Kanban** and overlaps `add-issue-to-kanban.yaml`. Its filter isn't API-readable — confirm in the UI whether it's dragging `open role` issues onto the Kanban as "No Status."
