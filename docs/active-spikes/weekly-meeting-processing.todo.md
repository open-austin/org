...
- [x] Move guarded issue creation from the private LifeOS CLI into `tools/issues/create.sh`.
- [x] Add dry-run-first Google Docs read and exact-match replacement tooling.
- [x] Document clone-local Google authentication in `.env.example` while allowing Aslan to reuse existing LifeOS credential/token files by path.
...
- [x] Fix the wrapper's standard-LifeOS credential fallback so paths discovered after `.env` loading are exported to the Python tool.
...
## Commit Boundary
- [ ] Commit the verified workflow and tools after Aslan explicitly approves the one unavoidable staging step for the new files. The pre-commit audit found no credential material; `.env`, OAuth token/credential JSON, `snapshot/`, and Python bytecode are ignored.
