...
## Purpose
Create a reusable Open Austin workflow for reconciling the weekly organizing meeting with the shared recurring Google Doc and GitHub without requiring contributors to adopt a private personal system.
...
## Tool Ownership
The org repo should own reusable Open Austin write mechanics. GitHub issue creation therefore moves here from a personal CLI. That personal CLI may continue to invoke the org sync and copy generated snapshots into the vault, but it should not remain the primary implementation of public issue writes.

Google Docs writes also belong here because they modify a shared Open Austin artifact. Authentication remains local and uncommitted. The public repo documents the required environment variables and provides its own OAuth path for other contributors; the wrapper automatically reuses Aslan's existing local credential and Open Austin token paths when present instead of copying secrets.
